...
Create the Supabase project via the web UI and run the full database schema so all subsequent issues have real tables, RLS, and seed data to work against. This is a HITL (human-in-the-loop) issue — it requires manual steps in the Supabase dashboard.

**Steps:**
1. Create a new Supabase project at supabase.com
2. Copy the project URL and anon/service-role keys into `.env.local`
3. Run the SQL below in the Supabase SQL editor
4. Create two Storage buckets: `post-images` (public) and `avatars` (public)
5. Insert 3–5 seed posts so the feed is not empty during development

**Full schema SQL to run in the Supabase dashboard:**
...
- [ ] Supabase project created, URL + anon key + service role key added to `.env.local`
- [ ] All tables exist and match the schema above
- [ ] All RLS policies applied
- [ ] `post-images` and `avatars` Storage buckets created (public read)
- [ ] `@supabase/supabase-js` installed and a basic query (e.g. `select * from locations limit 1`) runs from Next.js without error
- [ ] 3–5 seed posts with images exist so the feed renders during development
...
Foundation — enables all user stories that touch persistent data.
